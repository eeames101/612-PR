...
pop_fig
ggsave("fixed-pop-change.png", pop_fig, width = 8, height = 6, dpi = 300)
...
#Removed the "," so that the data can be run as a numerical value instead of as a character. 
social_geo_data <- social_geo_data %>%
     mutate(
        Over65 = as.numeric(gsub(",", "", Over65)),
        DisabledWorkers = as.numeric(gsub(",", "", DisabledWorkers)),
        pop_2018_2022 = as.numeric(gsub(",", "", pop_2018_2022))
    )
# Calculate the ratio of over65 to DI recipients
#changed the format 
ratio_data <- social_geo_data %>%
  mutate(
    over65Share = (Over65 / pop_2018_2022),
    disabledShare = (DisabledWorkers / pop_2018_2022)
  )
...
```{r}
#because the values were so similar we need to specify the limits more so that they work with the color scale.
#| label: figure-ratio
#| echo: true
#| output: true
...
relative_plot
ggsave("fixed-figure-ratio.png", relative_plot, width = 8, height = 6, dpi = 300)
# Save the figure using ggsave
